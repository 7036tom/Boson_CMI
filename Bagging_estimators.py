...
adaboost_model_train.fit(X_train[0:int(len(X_train)*Adapted_percentage)], Y_train[0:int(len(X_train)*Adapted_percentage)], sample_weights_train)#, sample_weight=sample_weights_train)



model_test_sklearn = KerasClassifier(build_fn=create_model_train,batch_size=400, nb_epoch=nb_epo, validation_split=0.2/Adapted_percentage)

adaboost_model_test= BaggingClassifier(base_estimator=model_test_sklearn, n_estimators=nb_esti, random_state=seed)

adaboost_model_test.fit(X_test[0:int(len(X_test)*Adapted_percentage)], Y_test[0:int(len(X_test)*Adapted_percentage)], sample_weight=sample_weights_test)#, sample_weight=sample_weights_test)
