...
def create_model(dr1, dr2, dr3, dr4):
    # create model
...
X1 = dataset[0:100000,2:17].astype(float)
X2 = dataset[0:100000,22:102].astype(float)
X = np.hstack([X1, X2])

Y = dataset[0:100000,0:1].astype(float)
...
param_grid = dict(dr1=dr1, dr2=dr2, dr3=dr3, dr4=dr4)
grid = GridSearchCV(estimator=model, param_grid=param_grid, n_jobs=1, cv =None)#, verbose=1)
