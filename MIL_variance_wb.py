...
for i in range(1):
    for train, test in kfold.split(X_train_best, Y_train):
        train=train[0:int(len(X_train_best)*Percentage)]

        print(j) 
        print("ieme fold")


        X_train_train = [X_train_best[train], X_train_best1[train], X_train_best2[train]]
        X_train_test = [X_train_best[test], X_train_best1[test], X_train_best2[test]]
        ...
        Classification_model_train[j] = Model([digit_a, digit_b, digit_c], out2)
        ...
        Classification_model_train[j].compile(optimizer=admax, loss='binary_crossentropy', metrics=['accuracy']) # Gradient descent
        ...
        # Fit the model     
        Classification_model_train[j].fit(X_train_train, Y_train_true[train],validation_data=(X_train_test, Y_train_true[test]), nb_epoch=100, class_weight=class_weight, batch_size=400, shuffle=True, verbose=1, callbacks=callbacks)#, class_weight=class_weight)
        j = j+1
...
for i in range(1):
    for train, test in kfold.split(X_test_best, Y_test):
        train=train[0:int(len(X_train_best)*Percentage)]

        print(j) 
        print("ieme fold")

        X_test_train = [X_test_best[train], X_test_best1[train], X_test_best2[train]]
        X_test_test = [X_test_best[test], X_test_best1[test], X_test_best2[test]]
        ...
        Classification_model_test[j] = Model([digit_a_test, digit_b_test, digit_c_test], out2_test)
        ...
        Classification_model_test[j].compile(optimizer=admax, loss='binary_crossentropy', metrics=['accuracy']) # Gradient descent
        #model_MIL.compile(optimizer=admax, loss='binary_crossentropy', metrics=['accuracy']) # Gradient descent
        ...
        # Fit the model     
        Classification_model_test[j].fit(X_test_train, Y_test_true[train],validation_data=(X_test_test, Y_test_true[test]), nb_epoch=100, class_weight=class_weight, batch_size=400, shuffle=True, verbose=1, callbacks=callbacks)#, class_weight=class_weight)
        j = j+1



Z_train1 = Classification_model_train[0].predict(X_test_true, batch_size=32, verbose=0)
Z_train2 = Classification_model_train[1].predict(X_test_true,  batch_size=32, verbose=0)
Z_train3 = Classification_model_train[2].predict(X_test_true, batch_size=32, verbose=0)
Z_train4 = Classification_model_train[3].predict(X_test_true, batch_size=32, verbose=0)
Z_train5 = Classification_model_train[4].predict(X_test_true, batch_size=32, verbose=0)

Z_train = np.copy(Z_train1)

for i in range(len(Z_train1)):
    Z_train[i][0] = (Z_train1[i][0] + Z_train2[i][0] + Z_train3[i][0] + Z_train4[i][0] + Z_train5[i][0])/5
    Z_train[i][1] = (Z_train1[i][1] + Z_train2[i][1] + Z_train3[i][1] + Z_train4[i][1] + Z_train5[i][1])/5

Z_test1 = Classification_model_test[0].predict(X_train_true, batch_size=32, verbose=0)
Z_test2 = Classification_model_test[1].predict(X_train_true, batch_size=32, verbose=0)
Z_test3 = Classification_model_test[2].predict(X_train_true, batch_size=32, verbose=0)
Z_test4 = Classification_model_test[3].predict(X_train_true, batch_size=32, verbose=0)
Z_test5 = Classification_model_test[4].predict(X_train_true, batch_size=32, verbose=0)

Z_test = np.copy(Z_test1)

for i in range(len(Z_test1)):
    Z_test[i][0] = (Z_test1[i][0] + Z_test2[i][0] + Z_test3[i][0] + Z_test4[i][0] + Z_test5[i][0])/5
    Z_test[i][1] = (Z_test1[i][1] + Z_test2[i][1] + Z_test3[i][1] + Z_test4[i][1] + Z_test5[i][1])/5
